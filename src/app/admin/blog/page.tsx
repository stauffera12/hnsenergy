...
function Blog() {
  ...
  const handleSearch = (e: any) => {
    ...
    const filterProduct = blogFilter.filter((item) =>
      toSlug(item.title).includes(searchText)
    );
  ...
  return (
    <>
      <Layout activate="blog">
        <>
          <Header title="Blog" strong className="relative max-md:fixed" />
          <Box className="h-[400px] w-full px-3 py-3">
            <div className="flex justify-between pb-3">
              <div className=" flex  items-center border-[1px] rounded-[8px] px-3 overflow-hidden">
                <input
                  ...
                  className=" outline-none py-2 p-3 "
            ...
            {isLoading ? (
              <div className="p-5 flex justify-center items-center">
                <img
                  src="/admin/loading.png"
                  alt="Loading Image"
                  className="animate-spin h-[80px]"
                />
              </div>
            ) : (
              <DataGrid
