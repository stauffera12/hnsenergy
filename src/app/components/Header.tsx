...
const Header = () => {
  ...
  const handleSearch = () => {
    const filteredStores: Store[] = Data.CategoryListData.features.filter(
      (store) => {
        const searchTermLower = searchTerm.toLowerCase();
        return store.storeId.toLowerCase().includes(searchTermLower);
      }
    );
      ...
      <div className="flex">
        <div className="flex gap-[20px] w-[85%] max-sm:w-[60%] max-lg:w-[70%] bg-[#102c4e] text-white pl-[110px] max-sm:pl-[20px] max-lg:pl-[40px] py-3">
          ...
          <div className="flex gap-[10px] items-center">
            <a
              href="https://www.linkedin.com/company/h-s-energy-llc/?viewAsMember=true"
              target="https://www.linkedin.com/company/h-s-energy-llc"
      ...
      <div className="bg-white">
        <div className="w-[100%] justify-center flex ">
          <div className=" w-[85%] max-sm:w-[90%] text-black flex text-[14px] text-left items-center justify-between gap-[30px] py-[20px] max-sm:pb-[20px] max-lg:pb-[20px] ">
            <Link href={"/"}>
              <img src="/logo.png" alt="H&N logo" className="max-sm:h-[75px]" />
            </Link>
            <div className="grid grid-cols-3 max-sm:hidden max-lg:hidden ">
              ...
              <div className="flex gap-[15px]">
                <img src="/address.png" alt="map Icon" />
                <div className="flex flex-col justify-center">
                  <span className=" font-normal">Office Address</span>
                  <span className=" font-bold">
                    2860 N.Santigo Blvd, Orange, CA 92867
                  </span>
                </div>
              </div>
            </div>
            ...
            />
            <div
              className={`fixed top-0 right-0 h-full w-full bg-[#af272f] z-50 transition-transform duration-300 ease-in-out transform ${
            ...
            >
              <div className=" flex flex-col text-start justify-start  py-4 gap-[30px] rounded-[7px] text-[13px] font-semibold bg-[#af272f] text-white">
                <div className="flex justify-end pr-[25px] pt-[10px]">
                  <button
                    onClick={handleOpen}
                    className=" bg-[#102c4e] rounded-full py-2 px-4 text-white text-2xl cursor-pointer"
                  >
                    X
                  </button>
                </div>
                  ...
                  <div className="w-[50%] flex flex-col text-start justify-start py-4 gap-[30px] rounded-[7px] text-[13px] font-semibold bg-[#af272f] text-white">
                    <Link
                      ...
                      className={`cursor-pointer ${
                        selected === "home" ? "border-b-2 border-white" : ""
                      }`}
                    ...
                    <Link
                      href={"/our-history"}
                      className={`cursor-pointer ${
                        selected === "our-history"
                          ? "border-b-2 border-white"
                          : ""
                      }`}
                      onClick={() => handleClick("our-history")}
                    >
                      Our History
                    </Link>
                    <a
                    ...
                    <Link
                      ...
                      onClick={() => handleClick("commercial-accounts")}
                    >
                      Commercial Accounts
                    </Link>
                    <Link
                      href={"/alternative-fuel"}
                      className={`cursor-pointer ${
                        selected === "alternative-fuel"
                          ? "border-b-2 border-white"
                          : ""
                      }`}
                      onClick={() => handleClick("alternative-fuel")}
                    >
                      Alternative Fuel
                    </Link>
                    <Link
                      href={"/careers"}
                      className={`cursor-pointer ${
                        selected === "careers" ? "border-b-2 border-white" : ""
                      }`}
